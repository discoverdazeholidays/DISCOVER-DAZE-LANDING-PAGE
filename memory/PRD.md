...
## GA4 form_submit conversion delivery fix (2025-12)
- RCA: page redirected to WhatsApp immediately after firing GA4 event -> beacon cancelled (esp. mobile current-tab nav) -> form_submit never reached GA4 -> Google Ads 0 conversions.
- Fix: tracking.js trackFormSubmit(payload,onDone) fires gtag('event','form_submit',{send_to:G-BBKY82CFQG,...,event_callback:finish}) + setTimeout(finish,1200) fallback; generate_lead once/session; Meta+Ads best-effort. BookingForm.handleBook passes the WhatsApp redirect as onDone so WA opens only AFTER the beacon is sent. postLead no longer fires tracking. Failed/404 submit still opens WA (prefilled) but fires NO conversion. Plain WA buttons fire 'contact', not form_submit.
- Verified iteration_8: /g/collect en=form_submit sent before wa.me nav (desktop+mobile); 1 form_submit per success (2 submits=2), generate_lead once; none on load/invalid/404/WA-click; gclid preserved. ID G-BBKY82CFQG unchanged.
